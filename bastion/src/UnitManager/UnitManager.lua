...
function UnitManager:CreateCustomUnit(token, cb)
    -- 创建异步包装的回调函数，只在Update时使用异步处理
    local asyncCb = function()
        return cb()  -- 在缓存访问时保持同步，避免递归
    end

    local unit = cb()  -- 初始同步调用以获取初始值
    local cachedUnit = Bastion.Cacheable:New(unit, asyncCb)
